...
## Summary
Creates `notifications` and `patient_portal_access` tables for a notification
abstraction layer (SMS/Email/WhatsApp templates) and patient portal access
management (enable/disable portal, generate access tokens, track last login).

## New Tables

### `notifications`
- id, clinic_id, patient_id (nullable for clinic-wide notifications)
- channel ('sms'|'email'|'whatsapp'|'in_app')
- template_key (e.g. 'appointment_reminder', 'payment_receipt', 'birthday')
- subject, body
- status ('pending'|'sent'|'failed'|'delivered')
- sent_at
- related_type (e.g. 'appointment', 'invoice', 'payment')
- related_id
- created_at

### `patient_portal_access`
- id, clinic_id, patient_id (unique per patient)
- is_enabled (boolean)
- access_token (unique, for portal login)
- last_login_at
- created_at, updated_at

## Security (RLS)
Membership-scoped. Owner/admin can delete.
*/
