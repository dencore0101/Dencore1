...
## Summary
Creates the expenses table for tracking clinic operating expenses
(rent, utilities, supplies, salaries, equipment, etc.) with categories
and optional recurring flags.
...
- category ('rent'|'utilities'|'supplies'|'salary'|'equipment'|'maintenance'|'marketing'|'lab'|'other')
- amount (numeric)
- expense_date (when the expense was incurred)
- vendor (who was paid)
- payment_method ('cash'|'bank'|'card'|'upi'|'cheque'|'other')
- is_recurring (boolean, for monthly/periodic expenses)
- notes
- created_by → auth.users
- created_at, updated_at

## Security (RLS)
Membership-scoped policies. Owner/admin can delete.
*/
